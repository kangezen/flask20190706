...
class TitanicModel:
    ...
    # self._context가 객체로 리턴 됨(여러 값이 리턴 될 수 있다)
    ...
    def hook_process(self, train, test) -> object :
        ...
        # test_id가 있어야 sklearn 에서 테스트가 가능하다
        ...
        self._test = t[1]  #test는 넘겨주지 않는다. overfit 방지
        return t[0]
    ...
    def hook_test(self, model, dummy):
        ...
        print('랜덤포르스트 활용한 검증 정확도 {} %'.format(self.accuracy_by_rforest(model, dummy)))
    ...
    def accuracy_by_decision_tree(train_X, train_Y, test_X, test_Y) -> str:
        ...
        tree_model.fit(train_X.vaules, train_Y.values)
        dt_prediction = tree_model.predict(test_X)  # 문제풀기
        # test_Y 답안지
